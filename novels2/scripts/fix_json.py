...
# 修复 world.json
with open('D:/AiProject/Node/novels2/world.json', 'r', encoding='utf-8') as f:
    data = json.load(f)
...
with open('D:/AiProject/Node/novels2/world.json', 'w', encoding='utf-8') as f:
    json.dump(world, f, ensure_ascii=False, indent=2)
print('world.json 已修复，字数:', len(world_str))

# 修复 characters.json
with open('D:/AiProject/Node/novels2/characters.json', 'r', encoding='utf-8') as f:
    data = json.load(f)
content = data['content']
match = re.search(r'```json\s*(.*?)\s*```', content, re.DOTALL)
if match:
...
with open('D:/AiProject/Node/novels2/characters.json', 'w', encoding='utf-8') as f:
