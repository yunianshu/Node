...
def review_chapter(chapter_number: int) -> dict:
    ...
    # 截取前1500字用于审查（减少token消耗，确保输出空间）
    content_sample = chapter_content[:1500]
    # 再截取中间和结尾部分
    mid_start = max(0, len(chapter_content) // 2 - 300)
    content_sample += "\n\n[中间部分...]\n\n" + chapter_content[mid_start:mid_start + 600]
    content_sample += "\n\n[结尾部分...]\n\n" + chapter_content[-500:]

    system = """你是一位资深恐怖灵异小说编辑，精通《神秘复苏》《我有一座恐怖屋》等作品的审稿标准。
你需要从多个维度审查章节质量，特别关注恐怖氛围、灵异规则逻辑和角色心理描写。
评分标准严格：9-10分优秀（恐怖氛围到位，杀人规律巧妙，求生逻辑严谨），7-8分良好，5-6分及格但需修改，低于5分需重写。
输出必须是合法的JSON格式。"""
...
5. 必须输出合法JSON"""
    ...
    content = call_mmx(system, prompt, max_tokens=8192, temperature=0.3)
