#!/usr/bin/env python3
"""
Reviewer Scheduler - 审查调度器
维持固定数量的并行reviewer agent，每个agent只审查2章后退出
"""
import subprocess
import sys
import time
...
SCRIPT = PROJECT_DIR / "scripts/reviewer.py"
CONFIG = PROJECT_DIR / "config.json"
LOG_FILE = PROJECT_DIR / "logs/reviewer_scheduler.log"
...
def get_pending_reviews():
    ...
    if DRAFT_DIR.exists():
        for f in DRAFT_DIR.glob("chapter_*.txt"):
            try:
                num = int(f.stem.split("_")[1])
                if num not in reviewed and f.stat().st_size > 1000:
                    pending.append(num)
            except (ValueError, IndexError):
                pass
...
def main():
    ...
    parser.add_argument("--per-agent", type=int, default=2, help="每个agent审查的章节数（默认2）")
    ...
    log("=" * 60)
    log(f"Reviewer Scheduler 启动 | 并行: {args.parallel} | 每agent: {args.per_agent}章")
    log("=" * 60)
    ...
    while True:
        ...
        batch = pending[:args.parallel * args.per_agent]
        if not batch:
            break
        ...
        log(f"本轮调度: {len(tasks)}个agent, 共{len(batch)}章, 剩余待审查{len(pending)}章")
