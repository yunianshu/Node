...
def generate_image(task):
    ...
    # 检查是否已存在
    existing = list(out_dir.glob(f"{name}_*.jpg"))
    if existing:
        log(f"[SKIP] {category}/{name} 已存在")
        return category, name, "exists", 0
    ...
    for attempt in range(3):
        ...
        try:
            ...
            elif "rate limit exceeded" in result.stderr or "RPM" in result.stderr:
                wait = 20 * (attempt + 1)
                log(f"[RPM] {category}/{name} 触发限流, 等待{wait}s后重试({attempt+1}/3)...")
                time.sleep(wait)
                continue
            else:
    ...
    log(f"[FAIL] {category}/{name} 重试耗尽")
    return category, name, "failed", 0


def main():
    print("=" * 60)
    print("世界观图片批量生成器 V2 启动")
    print("并发: 3 | 带RPM限流重试")
    print("=" * 60)
    ...
    for category, items in CATEGORIES.items():
        out_dir = IMAGES_DIR / category
        for name, _ in items:
            if list(out_dir.glob(f"{name}_*.jpg")):
                existing_count += 1

    print(f"已存在: {existing_count} 张, 待生成: {len(tasks) - existing_count} 张")
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        ...
        for future in as_completed(futures):
            ...
            if status == "success":
                ...
                total_failed += 1
            remaining = len(tasks) - total_success - total_exists - total_failed
            print(f"进度: 成功{total_success} | 已存在{total_exists} | 失败{total_failed} | 剩余{remaining}")

    elapsed_total = time.time() - start_time
    log("=" * 60)
    log(f"全部完成! 成功:{total_success} | 已存在:{total_exists} | 失败:{total_failed} | 耗时:{elapsed_total/60:.1f}分钟")
    log("=" * 60)
