...
class NovelCoordinator:
    ...
    # 通知
    # ------------------------------------------------------------------
    def _get_outline_progress(self) -> tuple[int, int]:
        """读取大纲进度：已生成章节数，缺失章节数"""
        try:
            if self.config.outline_file.exists():
                ...
                nums = [ch.get("chapter_number", 0) for ch in chapters]
                if nums:
                    max_num = max(nums)
                    missing = max_num - len(chapters)
    ...
    def _start_progress_pusher(self):
        ...
        def pusher():
            while self._running:
                ...
                outline_done, outline_missing = self._get_outline_progress()
                # 判断当前阶段
                if outline_done < self.config.total_chapters:
                    phase = "Planner（大纲生成）"
                    msg = (
                        f"《{self.config.title}》进度更新\n"
                        f"阶段：{phase}\n"
                        f"大纲：{outline_done}/{self.config.total_chapters} 章"
                    )
                    if outline_missing > 0:
                        msg += f"（缺失 {outline_missing} 章）"
                    pct = outline_done / self.config.total_chapters * 100
                    msg += f"\n完成度：{pct:.1f}%"
                    msg += f"\n初稿：{drafts}/{self.config.total_chapters} 章"
                    if drafts > 0:
                        msg += f"\n审查：{reviews}/{self.config.total_chapters} 章"
                else:
                    phase = "Writer（章节写作）"
                    pct = drafts / self.config.total_chapters * 100
                    msg = (
                        f"《{self.config.title}》进度更新\n"
                        f"阶段：{phase}\n"
                        f"大纲：✅ 2000/2000 章\n"
                        f"初稿：{drafts}/{self.config.total_chapters} 章（{pct:.1f}%）\n"
                        f"审查：{reviews}/{self.config.total_chapters} 章"
                    )
