...
def run_parallel_agents(agent_name: str, start: int, end: int, num_workers: int = 5) -> list:
    """并行运行多个Agent实例，将范围拆分为num_workers个子范围"""
    total = end - start + 1
    if total <= 0:
        return []
    ...
    log(f"[Coordinator] 并行启动 {len(ranges)} 个 {agent_name} 实例: {ranges}")

    results = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(ranges)) as executor:
        futures = {
            executor.submit(run_script, agent_name, "--start", str(s), "--end", str(e)): (s, e)
            for s, e in ranges
        }
        for future in concurrent.futures.as_completed(futures):
            ...
            try:
                ...
                log(f"[Coordinator] {agent_name} {s}-{e} 完成 (rc={rc})")
            except Exception as exc:
                log(f"[Coordinator] {agent_name} {s}-{e} 异常: {exc}")
...
def main():
    ...
    for batch_start in range(start_chapter, end_chapter + 1, batch_size):
        ...
        writer_results = run_parallel_agents("writer.py", batch_start, batch_end, num_workers=20)
        ...
        # 审查本批章节（并行20个Reviewer）
        if not args.skip_review:
            reviewer_results = run_parallel_agents("reviewer.py", batch_start, batch_end, num_workers=20)
