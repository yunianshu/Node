...
# 默认小说根目录（可以通过 --root 参数覆盖）
DEFAULT_NOVELS_ROOT = Path(__file__).parent.parent
PORT = 8889
...
def main():
    global NOVELS_PARENT, PORT

    parser = argparse.ArgumentParser(description="多本小说阅读器")
    parser.add_argument("--root", type=str, default=str(DEFAULT_NOVELS_ROOT),
                        help="小说根目录（默认：脚本所在目录的上级）")
    parser.add_argument("--port", type=int, default=PORT,
                        help=f"服务端口（默认：{PORT}）")
    args = parser.parse_args()

    NOVELS_PARENT = Path(args.root)
    PORT = args.port

    server = HTTPServer(("0.0.0.0", PORT), ReaderHandler)
    print("=" * 50)
    print("  多本小说阅读器已启动 (全局共享版)")
    print(f"  扫描目录: {NOVELS_PARENT}")
