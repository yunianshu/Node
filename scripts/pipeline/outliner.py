...
- 【爽点必释放】payoff_design 必须有完整的"期待→压制→反转→**碾压释放**"，且主角在其中**主动出击**（判断/抉择/出手），不能只被动承受。每章都要有读者能"爽到"的释放点。
...
- characters_involved 必须只含"角色设定"中已登记角色的规范名，每个名字是纯名字（如"周德茂""苏雯"）；禁止带括号注释（如"苏雯（记者）"）、"类别："前缀（如"反派：高建军"）或整句描述/群体名（如"受害者家属""神秘追踪者"）。
- 不得临时生造未登记的新命名角色。若剧情确需新角色，先用一个规范名（2-4字）写出，不得用称谓、职务或群体名占位。
